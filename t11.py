...
CLASSIFICATION_CONFIG = {
    ...
    "solo": {
        ...
        "confidence": 0.01, # Gera pontos para solo com confidence < 0.015
...
def generate_probability_maps(
    ...
    with rasterio.open(tiff_path) as src:
        ...
        # Verificar NoData e Banda Alpha
        ...
        with tqdm(total=total_chunks, desc="Processando chunks", unit="chunk") as pbar:
            for r_s in range(0, h, chunk_size):
                ...
                for c_s in range(0, w, chunk_size):
                    ...
                    # Criar máscara de NoData
                    if has_alpha:
                        alpha = src.read(alpha_band_idx, window=win)
                        mask = alpha >= 250
                    elif nodata_val is not None:
                        # Se qualquer banda RGB for NoData
                        mask = ~np.any(rgb == nodata_val, axis=-1)
                    else:
                        mask = np.ones((r_e - r_s, c_e - c_s), dtype=bool)
                    
                    # Achatar para processamento
                    mask_flat = mask.flatten()
                    
                    # Só processar se houver pixels válidos no chunk
                    if np.any(mask_flat):
                        rgb_u8 = np.clip(rgb, 0, 255).astype(np.uint8)
                        
                        # Extrair features apenas para pixels válidos
                        rgb_valid = rgb_u8.reshape(-1, 3)[mask_flat]
                        hsv_valid = cv2.cvtColor(rgb_u8, cv2.COLOR_RGB2HSV).reshape(-1, 3)[mask_flat]
                        exg_valid = calculate_exg(rgb_u8).reshape(-1, 1)[mask_flat]
                        
                        feat_valid = np.hstack([rgb_valid, hsv_valid, exg_valid])
                        ...
                        for class_name, config in classification_config.items():
                            # Criar mapa de probabilidade para o chunk (inicializado com 0 ou NoData)
                            prob_map_chunk = np.zeros((r_e - r_s, c_e - c_s), dtype=np.float32)
                            
                            # Preencher apenas os pixels válidos
                            prob_map_chunk.flatten()[mask_flat] = probas_valid[:, class_indices[config["label_value"]]]
                            ...
                            if confidence_threshold is not None and output_las_suffix is not None:
                                # Filtrar pixels válidos que atendem ao critério de confiança
                                ...
                                if len(rows_chunk) > 0:
                                    ...
                                    all_confidence_points[class_name].append(np.vstack([xs, ys, confidences]).T)
        ...
        for class_name, points_data_list in all_confidence_points.items():
            if points_data_list:
                ...
                las.x, las.y = all_points_for_class[:, 0], all_points_for_class[:, 1]
...
if __name__ == "__main__":
    ...
    with open(log_file_path, "a", encoding="utf-8") as f:
        f.write(f"\n{'#' * 80}\n# NOVA EXECUCAO: {time.strftime('%d/%m/%Y %H:%M:%S')}\n# SCRIPT: generate_probability_maps_and_points.py\n{'#' * 80}\n\n")
    ...
    try:
        features, labels = extract_features_for_training(INPUT_IMAGE_PATH, CLASSIFICATION_CONFIG, BUFFER_SIZE_METERS, log_file_path)
        if len(features) > 0:
            model = train_model(features, labels, log_file_path)
            generate_probability_maps(INPUT_IMAGE_PATH, model, CLASSIFICATION_CONFIG, OUTPUT_DIR, log_file_path)
        else:
            log_message("Nao foi possivel extrair features para treinamento.", log_file_path)
