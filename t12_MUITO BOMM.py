...
CLASSIFICATION_CONFIG = {
    ...
    "solo": {
        ...
        "confidence": 0.00005, # Gera pontos para solo com confidence < 0.00005 (ajuste conforme necessário)
...
def generate_probability_maps(
    ...
    with rasterio.open(tiff_path) as src:
        ...
        # Inicializar mapas de probabilidade com NaN
            ...
            for r_s in range(0, h, chunk_size):
                ...
                for c_s in range(0, w, chunk_size):
                    ...
                    if np.any(mask):
                        ...
                        # Extrair features apenas para pixels válidos (usando indexação 2D)
                        ...
                        for class_name, config in classification_config.items():
                            ...
                            # Criar mapa de probabilidade para o chunk (inicializado com NaN)
                            prob_map_chunk = np.full((r_e - r_s, c_e - c_s), np.nan, dtype=np.float32)
                            
                            # *** FIX: Usar indexação 2D booleana diretamente em vez de flatten() ***
                            # flatten() retorna COPIA, então a atribuição se perde.
                            # mask é 2D (r_e-r_s, c_e-c_s), probas_valid[:, class_idx] é 1D
